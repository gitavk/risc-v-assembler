...
# 1. Замените каждый пробел на число символов “_” в исходной строке. Если число больше 6, то укажите его остаток от делени
# я на 11
# 2. Замените все подстроки OOOO на подстроки WWWW
# 3. Перевести все согласные ['B', 'C', 'D', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'V', 'W', 'X
# ', 'Z'] в верхний регистр
...
main:
    ...
    # Transformation 1: Replace spaces with N underscores
    ...
    # Transformation 2: Replace "0000" with "WWWW"
    la a0, work_buffer    # a0 = source buffer
    mv a1, s0             # a1 = length
    la a2, input_buffer   # a2 = destination buffer
    call replace_oooo_with_wwww
    mv s0, a0             # s0 = new length

    # Transformation 3: Uppercase consonants
    la a0, input_buffer   # a0 = buffer address
    mv a1, s0             # a1 = length
...
count_done:
    li t4, 6
    blt t0, t4, single_cnt # if counter > 6, single counter
    li t4, 11
    rem t0, t0, t4
single_cnt:
...
# Output: a0 = new length
replace_oooo_with_wwww:
    mv t0, a2             # t0 = dest pointer
    li t1, 'O'            # t1 = 'N' character
replace_oooo_loop:
    li t2, 4              # t2 = 4
    blt a1, t2, copy_rest # if remaining < 4, copy rest

    # Check if next 4 characters are "OOOO"
    ...
    # Found "OOOO" - replace with "WWWW"
    li t3, 'W'            # t3 = 'I' character
    sb t3, 0(t0)          # write 'I'
    sb t3, 1(t0)          # write 'I'
    sb t3, 2(t0)          # write 'I'
    sb t3, 3(t0)          # write 'I'
    ...
    j replace_oooo_loop   # continue loop
...
copy_rest:
    beqz a1, replace_oooo_done  # if length == 0, done
    lbu t3, 0(a0)         # t3 = current character
    sb t3, 0(t0)          # write character
    addi t0, t0, 1        # dest += 1
    addi a0, a0, 1        # source += 1
